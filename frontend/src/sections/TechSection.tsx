...
export const TechSection: React.FC = () => {
  ...
  return (
    <section id="technologies" className="py-12 sm:py-14 md:py-16 bg-[#F7F9FC] relative overflow-hidden">
        ...
        <div className="flex items-center justify-center flex-wrap gap-2 mb-8">
          {categories.map((tab) => (
            <button
              ...
              className={`px-4 py-2 sm:px-5 sm:py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 cursor-pointer ${
                activeCategory === tab.id
                  ? 'bg-gradient-to-r from-[#1769E0] to-[#6C3FE8] text-white shadow-glow-blue'
            ...
            <div key={category.id} className="space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 pb-2.5 border-b border-slate-200">
                <div className="flex items-center gap-2.5">
                  <h3 className="text-lg sm:text-xl font-bold text-[#0B1B3A]">{category.name}</h3>
                  <span className="text-[10px] font-semibold text-[#1769E0] bg-blue-50 px-2 py-0.5 rounded-full">
                    {category.badge}
                  </span>
                </div>
