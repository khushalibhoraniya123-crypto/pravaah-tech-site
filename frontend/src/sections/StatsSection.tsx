...
export const StatsSection: React.FC = () => {
  ...
  return (
    <section className="py-8 sm:py-9 md:py-10 bg-gradient-to-r from-[#07152F] via-[#0B1B3A] to-[#07152F] text-white relative overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-3.5 xs:px-4 sm:px-6 lg:px-8 xl:px-12 relative z-10">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 xs:gap-3.5 sm:gap-6">
          {stats.map((stat: StatItem, idx: number) => {
            ...
            return (
              <div
                key={idx}
                className="flex flex-col items-center text-center p-3 xs:p-4 sm:p-5 rounded-xl xs:rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/10 transition-all duration-300 group"
              >
                <div className="w-8 h-8 xs:w-10 xs:h-10 sm:w-11 sm:h-11 rounded-lg xs:rounded-xl bg-gradient-to-tr from-[#1769E0] to-[#6C3FE8] text-white flex items-center justify-center mb-2 xs:mb-3 shadow-glow-blue group-hover:scale-105 transition-transform duration-300">
                  <Icon className="w-4 h-4 xs:w-5 xs:h-5" />
                </div>

                <div className="text-xl xs:text-2xl sm:text-3xl md:text-4xl font-black gradient-text-blue-purple mb-0.5 xs:mb-1">
                  <AnimatedCounter target={stat.numberOnly} suffix={stat.suffix} />
                </div>

                <div className="text-[11px] xs:text-xs sm:text-sm font-bold text-white mb-0.5">
                  {stat.label}
                </div>

                <p className="text-[9.5px] xs:text-[11px] text-slate-400 max-w-[200px] leading-relaxed">
                  {stat.description}
                </p>
              </div>
            );
